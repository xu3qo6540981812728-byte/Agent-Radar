  ...
  contract: { exclusive: 100, general: 40 } };
        ...
        const DIMENSIONS = {
          personality: {
            ...
            desc: "決定溝通頻率、語氣與心理攻防的切入點",
            options: [
              { id: 'tiger', label: '老虎型 (掌控型)', desc: '語速快、聲音大、常打斷、喜下指令、只問結果' },
              { id: 'owl', label: '貓頭鷹型 (分析型)', desc: '重細節、愛筆記、比對資料、重法規、情感冷靜' },
              { id: 'peacock', label: '孔雀型 (社交型)', desc: '熱情愛聊、肢體豐富、重面子、喜聽好話、情緒化' },
              { id: 'koala', label: '無尾熊型 (和平型)', desc: '慢條斯里、猶豫不決、常問家人、怕衝突、配合度高' }
            ]
          },
          motivation: {
            title: "維度二：售屋動機",
            icon: <IconActivity className="w-6 h-6" />,
            desc: "判斷急迫性與痛點，決定談判籌碼的大小",
            options: [
              { id: 'change', label: '換屋需求型', desc: '換大換小或工作調動，有明確時間表與剛性需求' },
              { id: 'cash', label: '急需資金型', desc: '解決債務或周轉，急迫性最高，痛點最明顯' },
              { id: 'asset', label: '閒置去化型', desc: '繼承或多餘資產，無急迫壓力，理性配置考量' },
              { id: 'test', label: '試探水溫型', desc: '單純賣賣看，心態隨緣，價格不漂亮就不賣' }
            ]
          },
          ownerType: {
            title: "維度三：屋主種類",
            icon: <IconUsers className="w-6 h-6" />,
            desc: "判斷決策模式是偏向「情感價值」還是「數據邏輯」",
            options: [
              { id: 'normal', label: '一般客', desc: '普通民眾，情感連結深，易受「稟賦效應」影響' },
              { id: 'investor', label: '投資客', desc: '經驗豐富，看重投報率與成本，決策理性' }
            ]
          },
          condition: {
            title: "維度四：房屋條件",
            icon: <IconHome className="w-6 h-6" />,
            desc: "判斷產品本身的優劣勢與市場定位",
            options: [
              { id: 'perfect', label: '完美屋況 (A案)', desc: '無需整理即可入住，賣相極佳，市場接受度高' },
              { id: 'needsWork', label: '需整理 (B案)', desc: '需小修補或粉刷，結構無虞，需靠軟裝或想像力銷售' },
              { id: 'flaw', label: '瑕疵較多 (C案)', desc: '路沖、凶宅或屋況極差，客群受限，需特殊通路' },
              { id: 'complex', label: '條件複雜 (客製)', desc: '產權複雜或格局極度特殊，需專業法務或特殊買方' }
            ]
          },
          price: {
            title: "維度五：價格狀況",
            ...
            desc: "判斷屋主對行情的認知偏差程度",
            options: [
              { id: 'market', label: '市場價 (行情)', desc: '符合實價登錄，合理區間，成交阻力最小' },
              { id: 'high', label: '尚可價 (略高)', desc: '略高於行情，預留議價空間，屬正常心態' },
              { id: 'challenge', label: '挑戰價 (極高)', desc: '明顯高於行情，憑藉屋況或情感開價，需時間磨合' },
              { id: 'urgent', label: '急售價 (低於行情)', desc: '低於行情，尋求快速成交，通常伴隨強烈動機' }
            ]
          },
contract: {
    title: "特別加權：委託型態",
        icon: <IconTarget className="w-6 h-6" />, 
    desc: "決定案件的掌控權與成交的安全係數",
    options: [
      { id: 'general', label: '一般委託 (多窗)', desc: '競爭激烈，資訊混雜，無法掌控同業動態，成交不一定在自己手上' },
      { id: 'exclusive', label: '專任委託 (單一)', desc: '單一窗口，屋主接收資訊單純，擁有完整議價權與佈局時間，成交機率極高' }
    ]
  }
};
        
        // --- 3. 雷達圖組件 (SVG) ---
        const RadarChart = ({ selections }) => {
  // 確保順序與 DIMENSIONS 一致
  const dimensionKeys = Object.keys(DIMENSIONS);
  const count = dimensionKeys.length; // 變成 6
  ...
  const center = size / 2;
  const radius = 90;
  const angleStep = (Math.PI * 2) / count; // 自動計算角度
  const labels = ["性格", "動機", "屋主", "屋況", "價格", "委託"]; // 簡化標籤
      ...
      <svg width={size + 80} height={size + 40} viewBox={`0 0 ${size + 80} ${size + 40}`}>
        <g transform="translate(40, 20)">
          ...
          {gridPolygons.map((pts, i) => (
            <polygon key={i} points={pts} fill={i === 4 ? "#f8fafc" : "none"} stroke="#cbd5e1" strokeWidth="1" strokeDasharray={i < 4 ? "4 4" : ""} />
          ))}
          ...
          {/* 文字標籤 */}
          {labelCoords.map((l, i) => (
            <text key={i} x={l.x} y={l.y} textAnchor="middle" dominantBaseline="middle" fontSize="12" fill="#475569" fontWeight="bold">
              {l.text}
            </text>
          ))}
        </g>
  ...
  return score;
        ...
        const generateAnalysis = (selections) => {
          ...
          psychology = "【和平型人格解析】\n此類屋主核心需求是「安全感」。他最怕做錯決定被家人埋怨。注意：無尾熊雖然好講話，但「決策極慢」，容易因為一句家人的話就反悔。"; // 增加這句警語
