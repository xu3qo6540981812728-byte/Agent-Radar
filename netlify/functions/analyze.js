...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    if (isGoodProduct) {
      // 雖然是好物件，要區分是「專任」還是「一般」
      if (contract === 'exclusive') {
        // 情況一：專任 + 好物件 = 真正的黃金 A 案 (分數高)
        teamStrategy += "目前的局勢是「眾星拱月 (黃金 A 案)」。價格具競爭力且為專任獨家，這是成交率與獎金最高的類型。\n";
        teamStrategy += "• 給開發的建議：您掌握了市場稀缺資源。請善用「專任」優勢，讓價格「收得漂亮」。\n";
        teamStrategy += "• 團隊引導：告訴同事這是「秒殺件」，鼓勵大量帶看，並強調「只有我能談」，創造排隊看屋的氛圍。";
      } else {
        // 情況二：一般 + 好物件 = 殺戮戰場 (解決 59 分矛盾的問題)
        teamStrategy += "目前的局勢是「兵家必爭 (一級戰區)」。物件條件極佳，但為一般委託，全市場都在搶，風險極高。\n";
        teamStrategy += "• 給開發的建議：這是典型的「速度戰」。您的分數被合約型態拉低了，代表容易做白工。要避免被同業捷足先登。\n";
        teamStrategy += "• 團隊引導：對同事要強調「要快」，請同事有喜歡的買方趕快一起加油，不要猶豫。";
      }
